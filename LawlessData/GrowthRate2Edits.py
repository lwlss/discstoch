...
def getBlobs(image,bk,showIms=False,DX=25,DY=25):
    ...
    cv.Copy(im,im2) ##NOT SURE ABOUT THIS STEP
    ...
    storage=cv.CreateMemStorage()    #WHY?
    element_shape = cv.CV_SHAPE_RECT #WHY? 
    for x in xrange(1,10):
        cv.Dilate(dst,dst) #same input and output
        if showIms:
            cv.SaveImage("5ShowImages%05d.png"%imnum,dst) #
            imnum+=1
        cv.Erode(dst,dst) ##NOT SURE ABOUT THIS STEP 
        if showIms:
            cv.SaveImage("6ShowImages%05d.jpg"%imnum,dst)
            imnum+=1
    ...
    while contour:
        #print(contour)
        area=cv.ContourArea(contour) #area of blob 
        print(area*math.pi*4) 
        i,(x,y),radius=cv.MinEnclosingCircle(contour)
        print((radius*2*math.pi)**2) #this should be roughly equal to the above if full circle
        print(" ") #BUT WHERE TO SET THE CUT-OFF?? 
        if area>15.0: #WHY THIS NUMBER? 
...
##Where these numbers manually/arbitrarily chosen? 
# Final photo number...
FINALPHOT=35
...
for f in folders:
    ...
    blbs=getBlobs(os.path.join(fullpath,f,finalphoto),bk,DX=DX,DY=DY) #showIms=True,
    ...
    locs=numpy.zeros((NBlbs,4),numpy.int)  #what the 4 columns for? 
    ...
    for imno in xrange(0,FINALPHOT): #iterating through all image numbers less than 36
        ...
        for blb in blbs:
            ...
            maskedarea=cv.CountNonZero(black) #Does this give the area!?!?!? 
            #print maskedarea,cv.GetSize(black)
            cv.ResetImageROI(black)
            res[imno,blbno]=maskedarea
            locs[blbno-1]=[ROI[0]-DX,ROI[1]-DY,ROI[2],ROI[3]] #DONT UNDERSTAND THIS BIT;
            #must be generating final blob images 
            blbno+=1
        blbno=0
        for blb in blbs:
