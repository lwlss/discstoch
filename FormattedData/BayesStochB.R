...
dataLik<-function(x,t,y,log=FALSE,...){ #dataLik(xmat[j,],t=times[i+1],y=data[i,],log=FALSE) 
...
mcmc = function(p,tune,iters,thin,mLLik,th,pmin,pmax){
  ...
  for (i in 1:iters) {
    ...
    for (j in 1:thin) {
      ...
      B=rbinom(1,1,thprob[2])
      llprob=mLLik(th=c(thprob[1],"B"=B))
...
#switchN=65
#switchN=round(max(calibrated_area,na.rm=TRUE))+1
switchN=1
noiseSD=10

# Step Function for pfMLLik
stepSim=function(x0=1, t0=0, deltat=1, th=c(3,1))  simDt(K=15000,th[1],th[2],x0,switchN,t0,t0+deltat) #fix K and change dimension of th
